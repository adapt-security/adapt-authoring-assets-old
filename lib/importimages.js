...
export default async function importImages(req, res, next) {
  const importImage = async function (image, imageMeta) {
    const dbData = Object.assign(req.apiData.data, imageMeta, { type: 'image' });
    //save file to repository
    const filename = await this.repository.save(image);
    try {
      //insert file to mongodb
      const result = await this.insert(Object.assign(dbData, {
        path : filename,
        size: AssetsUtils.getFileSize(image.filepath),
      }));
      //rename saved image
      const newFilePath = await this.repository.rename(filename, image.mimetype, result._id);
