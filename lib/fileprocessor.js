...
class FileProcessor {
    ...
    async createImageThumb(imagePath) {
            let thumbPath = AssetsUtils.getThumbPath(imagePath, this.thumbSuffix);
    ...
    async createVideoPoster(videoFile) {
            ...
            try {
                ...
                await this.createVideoScreenshot(videoFile, size, this.thumbSuffix).then((file) => {
                    result = file
                });
            } catch (error) {
                throw error;
    ...
    async createVideoScreenshot(videoFile, size, suffix) {
            let filename = `${path.basename(videoFile, '.mp4')}${suffix}.png`;
            try {
