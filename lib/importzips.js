...
const importZips = function(req, res, next) {
    ...
    const unZip = async(zip) =>  {
      try {
        let hash = await AssetsUtils.getHash(zip.path);
        
        //dir where the files from zip file extract to
        const extractDir = `${this.getConfig("uploadTempDir")}/${hash}`;

        await fs.ensureDir(extractDir, (err) => {
          return new Error(`${App.instance.lang.t('error.failedtocreatedir')} ${err}`)
        });
        ...
        return new Error(`${App.instance.lang.t("error.failedunzip")} ${err}`);
      }
    }
