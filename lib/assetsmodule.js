...
class Assetsmodule extends AbstractApiModule {
  ...
  importResponse(req, res, next) {
    if(req.query.unzip === "true" && req.zips.length) {
      ...
      const isTempDirDeleted = AssetsUtils.removeDir()
        .then((result) => {
          if(result !== true) {
            res.sendError('Failed to delete dir');
            return;
          }
        });
    }
    const results = req.results.filter(e => e);//Clear null element
    res.send(results);
  }
  /**
