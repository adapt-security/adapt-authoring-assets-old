...
class Assetsmodule extends AbstractApiModule {
  ...
  async setValues() {
    ...
    this.repositoryPath = path.resolve(this.app.rootDir, this.getConfig("uploadDir"));
    this.tempUploadPath = path.resolve(this.app.rootDir, this.getConfig("uploadDir"));
 ...
 */
  async importResponse(req, res, next) {
    if (req.query.unzip === "true" && req.zips.length) {
      ...
      const isTempDirDeleted = await AssetsUtils.removeDir(this.tempUploadPath);
