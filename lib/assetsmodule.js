...
class Assetsmodule extends AbstractApiModule {
  ...
  async setValues() {
    ...
    const [authored, tags, mongodb] = await this.app.waitForModule("authored", "tags");
  ...
  retrieveAsset(req, res, next) {
    this.find({ _id: req.params["id"] }, {})
      ...
      .catch((e) => {
        res
          .status(500)
          .send(`${App.instance.lang.t("error.retrievingAsset")} ${e}`);
      });
  }
  ...
  deleteAsset(req, res, next) {
    console('deleting assets');
    //todo delete asset file

    this.delete({ _id: req.params["id"] }, {})
    .then(() => {
      res.status(200).send("Asset is deleted");
    })
    .catch((e) => {
      console("failed deleting assets");
      res.status(500).send(e);
    });
  }
}
