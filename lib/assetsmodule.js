...
class Assetsmodule extends AbstractApiModule {
    ...
    this.routes = [
      ...
      },
      {
        ...
        handlers: {
          ...
          patch: [this.replaceAssetFile.bind(this)],
          delete: [this.deleteAsset.bind(this)],
        },
        permissions: { patch: writePerms, put: writePerms, delete: writePerms },
      },
      {
        route: "/serve/:id",
        handlers: {
  ...
  */
  async replaceAssetFile(req, res, next) {
    //TODO: needs to be refactored
    const acceptableTypes = new Set(this.getConfig("acceptableTypes"));
    const asset = await this.find({ _id: req.params["id"] }, {});
    if(!asset) res.status(500).send(App.instance.lang.t("error.noassetsfound"));
    const self = this;
      ...
      for (const file of Object.values(files)) {
        ...
        if (file && !file.length) {
          createNewDBData(file.type, file, res)
            .then((newDBData) => {
              if(newDBData !== {} || newDBData !== null || newDBData !== undefined) {
                this.update({ _id: req.params["id"] }, newDBData)
          ...
          res.messages = res.messages || [];
          res.messages.push(App.instance.lang.t("error.notsuppertedfiletype"));
        }
      }
      //This function will create new object to update record in mongodb
      async function createNewDBData(fileType, file, res) {
        let newData = {};
        ...
        if (acceptableTypes.has(fileType)) {
          switch (fileType) {
            ...
            case "image/gif":
              ...
              newData.path = filenameHash;
              newData.size = file.size;
              break;
            case "audio/mpeg":
              const audiometadata = await fileProcessor.retrieveMediaMetadata(video.path);
              newData.path = filenameHash;
              newData.size = file.size;
              newData.metadata = audiometadata;
              break;
            case "video/mp4":
              const videometadata = await fileProcessor.retrieveMediaMetadata(video.path);
              ...
              newData.path = filenameHash;
              newData.size = file.size;
              newData.metadata = videometadata;
              newData.poster = videoPoster;
              break;
  ...
  async deleteAsset(req, res, next) {
    const result = await this.find({ _id: req.params["id"] });
    ...
    if(fullPath) {
      const pathRecords = await this.find({ path: result[0].path });
      const hasMultiPointer = pathRecords.length > 1;
      if(!hasMultiPointer) { //If only sigle assets points to a file, will delete this file from the repository
      ...
      const isAssetRecordDeleted = await this.delete({ _id: req.params["id"] });
