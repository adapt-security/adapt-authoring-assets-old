...
class Assetsmodule extends AbstractApiModule {
  ...
  async setValues() {
    ...
    this.routes = [
      ...
      },
      {
        route: "/poster/:id",
        handlers: {
          put: [this.replaceposterimage.bind(this)],
        },
        permissions: { put: writePerms },
      },
      {
        route: "/serve/:id",
        handlers: {
  ...
  /**
 * To replace poster image
 */
  async replaceposterimage(req, res, next){
    ...
    form.parse(req, (err, fields, files) => {
      ...
      for (const file of Object.values(files)) {
        ...
        if (!file.type.includes('image')) {
          ...
          res.status(500).send(App.instance.lang.t("error.notimagefile"));
          break;
        }
        if (file && !file.length) {
