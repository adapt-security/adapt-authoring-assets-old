...
*/
 ...
 /** Delete asset via API */
    ...
    if (result[0]) {
      ...
      if (fullPath && typeof fullPath === 'string') {
        ...
        if (!hasMultiPointer && result[0].path !== result[0].poster) {
          ...
          if (isFileDeleted) {
            this.deleteAssetDirs(result[0].path);
            msg = await this.deleteRecord(req.params["_id"]);
          } else {
  ...
  /** Delete empty asset dirs*/
  deleteAssetDirs(filepath) {
    try {
      const assetDirs = AssetsUtils.nameToDirPaths(filepath);
      this.repository.removeAssetDir(`${this.repositoryPath}/${assetDirs[0]}/${assetDirs[1]}`);
      this.repository.removeAssetDir(`${this.repositoryPath}/${assetDirs[0]}/`);
    } catch (error) {
      throw new Error(new Error(this.app.lang.t('error.faileddeletedir')))
    }
  }
  /**
   * 
