...
export default async function replaceAssetFile(req, res, next) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    for (const file of Object.values(files)) {
      ...
      if (asset[0].type && !file.mimetype.includes(asset[0].type)) {
      ...
      if (file && !file.length) {
        try {
          req.apiData.data = await createNewDBData(file.mimetype, file, req.apiData.query);
    ...
    async function createNewDBData(fileType, file, data) {
      ...
      const filename = await self.repository.save(file.filepath, fileType);
      ...
      if (acceptableTypes.has(fileType)) {
        switch (fileType) {
          ...
          case "audio/mpeg":
            ...
            const audiometadata = await fileProcessor.retrieveMediaMetadata(file.filepath);
            newObject.path = filename;
            newObject.size = file.size;
          ...
          case "video/mp4":
            ...
            const videometadata = await fileProcessor.retrieveMediaMetadata(file.filepath);
